...
import {
  ...
  AlertCircle, Eye, Copy, Lock, Send, ClipboardList,
} from "lucide-react";
...
export default function InvoiceEditor() {
  ...
  const statusClass = form.status === "final"
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      {isReadOnly && (
        <div className="bg-amber-50 border-b border-amber-200 px-4 py-2.5 flex items-center gap-2 text-sm text-amber-800">
          ...
          <span>Anda hanya memiliki akses <strong>baca saja</strong> untuk Invoice. Perubahan tidak akan tersimpan.</span>
        </div>
      )}
      {needsApproval && (
      ...
      <div className="max-w-screen-2xl mx-auto px-3 sm:px-4 py-5 flex gap-5 items-start">
        ...
        <div className={`flex-1 min-w-0 space-y-4 ${mTab === "preview" ? "hidden xl:block" : ""}`}>
          ...
          {/* ▸ Buat dari RAB — hanya saat buat baru */}
          {!isEdit && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 flex items-center justify-between gap-4">
              <div className="min-w-0">
                ...
                <p className="text-xs text-amber-700 mt-0.5 leading-relaxed">
                  Pilih RAB yang sudah ada untuk mengisi otomatis perihal, item, instansi, dan data penerima.
                </p>
                {form.sourceRabNomor && (
                  <p className="mt-1.5 text-xs font-medium text-amber-800 bg-amber-100 rounded-lg px-2.5 py-1 inline-flex items-center gap-1.5">
                    ...
                    Diisi dari RAB {form.sourceRabNomor}
                  </p>
                )}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <div className="flex gap-2">
                  <input className={inp} value={form.nomor}
                    ...
                    placeholder="Contoh: 001/APT/VI/2026" />
              ...
              <div className="sm:col-span-2">
                ...
                <input className={inp} value={form.perihal}
                  ...
                  placeholder="Contoh: Pembayaran Jasa Instalasi Listrik" />
                <Hint>Deskripsi singkat tujuan invoice ini.</Hint>
              </div>
              ...
              <div>
                ...
                <input type="date" className={inp} value={form.jatuhTempo}
                  onChange={e => set("jatuhTempo", e.target.value)} />
                <Hint>Batas waktu pembayaran untuk pelanggan.</Hint>
              </div>
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Hint>Tampil di header dokumen: "Gresik, 22 Juni 2026"</Hint>
              </div>
          ...
          {/* ▸ Section: Tagihan Kepada */}
          <Section
            icon={<User className="w-5 h-5" style={{ color: "#10B981" }} />}
            title="Tagihan Kepada"
            subtitle="Data pelanggan atau klien yang menerima invoice"
            color="#10B981"
            open={openSec.tagihan}
            onToggle={() => toggleSec("tagihan")}
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <Label>Nama Kontak</Label>
                <input className={inp} value={form.tagihan.nama}
                  onChange={e => setTagihan("nama", e.target.value)}
                  placeholder="Bapak / Ibu Andi" />
              </div>
              <div>
                <Label>Nama Perusahaan</Label>
                <input className={inp} value={form.tagihan.perusahaan}
                  onChange={e => setTagihan("perusahaan", e.target.value)}
                  placeholder="PT. Pelanggan Jaya" />
              </div>
              <div className="sm:col-span-2">
                <Label>Alamat</Label>
                <textarea className={`${inp} !h-auto`} rows={2} value={form.tagihan.alamat}
                  onChange={e => setTagihan("alamat", e.target.value)}
                  placeholder="Jl. Contoh No. 1, Gresik" />
              </div>
              <div>
                <Label>Telepon</Label>
                <input className={inp} value={form.tagihan.telp}
                  onChange={e => setTagihan("telp", e.target.value)}
                  placeholder="08xx-xxxx-xxxx" />
              </div>
              <div>
                <Label>Email</Label>
                <input className={inp} value={form.tagihan.email}
                  onChange={e => setTagihan("email", e.target.value)}
          ...
          <Section
            ...
            extra={
              <button onClick={() => setShowCopy(true)}
                ...
                <Copy className="w-3.5 h-3.5" /> Salin dari dokumen lain
              </button>
            }
          >
          ...
          <Section
            ...
            color="#EC4899"
            open={openSec.pembayaran}
            onToggle={() => toggleSec("pembayaran")}
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <Label>Nama Bank</Label>
                <input className={inp} value={form.pembayaran.bank}
                  onChange={e => setPembayaran("bank", e.target.value)}
                  placeholder="Contoh: BCA, Mandiri, BNI, BRI" />
              </div>
              <div>
                <Label>Nomor Rekening</Label>
                <input className={inp} value={form.pembayaran.noRek}
                  onChange={e => setPembayaran("noRek", e.target.value)}
                  placeholder="1234 5678 9012" />
              </div>
              <div className="sm:col-span-2">
                <Label>Atas Nama</Label>
                <input className={inp} value={form.pembayaran.atasNama}
              ...
              <div className="sm:col-span-2 text-xs text-slate-400 flex items-start gap-1.5">
                <AlertCircle className="w-3.5 h-3.5 mt-0.5 shrink-0" />
                <span>Info pembayaran akan tampil di invoice di bawah tabel item agar pelanggan mudah membayar.</span>
              </div>
            </div>
          </Section>
          ...
          <Section
            ...
            subtitle="Opsional — pesan tambahan dan kalimat penutup di invoice"
            color="#6B7280"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <textarea className={`${inp} !h-auto`} rows={3} value={form.catatan}
                  ...
                  placeholder="Contoh: Pembayaran via transfer. Harap menyertakan nomor invoice sebagai berita pembayaran." />
              </div>
            </div>
          </Section>
    ...
    <div ref={setNodeRef} style={style}
      ...
      <div className="hidden sm:grid items-center gap-2 px-3 py-2.5"
        ...
        <select value={item.satuan} onChange={e => onChange({ satuan: e.target.value })}
          className="text-sm border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30 bg-white text-center">
          {SATUAN_OPTIONS.map(s => <option key={s}>{s}</option>)}
        </select>
        <input type="number" min="0" step="0.01" value={item.volume}
          onChange={e => onChange({ volume: e.target.value })}
          className="text-sm text-center border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30 w-full" />
        <input type="number" min="0" step="1" value={item.hargaSatuan}
          onChange={e => onChange({ hargaSatuan: e.target.value })}
          placeholder="0"
          className="text-sm text-right border border-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30 w-full" />
      ...
      <div className="sm:hidden p-3 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={() => setEditingNama(true)}
            className="flex-1 text-left text-sm border border-slate-200 rounded-lg px-2.5 py-1.5 truncate hover:border-blue-400 hover:bg-blue-50/40 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500/30 min-w-0">
            {item.nama
              ? <span className="text-slate-900">{item.nama}</span>
        ...
        <div className="flex items-center gap-2 pl-8">
          <input type="number" min="0" value={item.volume}
            onChange={e => onChange({ volume: e.target.value })}
            className="w-16 text-sm text-center border border-slate-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
          <select value={item.satuan} onChange={e => onChange({ satuan: e.target.value })}
            className="text-sm border border-slate-200 rounded-lg px-2 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30">
            {SATUAN_OPTIONS.map(s => <option key={s}>{s}</option>)}
          </select>
          ...
          <input type="number" min="0" value={item.hargaSatuan}
            onChange={e => onChange({ hargaSatuan: e.target.value })}
            placeholder="Harga"
            className="flex-1 text-sm text-right border border-slate-200 rounded-lg px-2.5 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500/30" />
      ...
      <div className="bg-white rounded-2xl w-full max-w-md max-h-[80vh] flex flex-col shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Salin daftar item dari Invoice atau RAB yang sudah ada</p>
          </div>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-700 transition-colors text-lg">✕</button>
        </div>

        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          <input className={inp} placeholder="🔍 Cari nomor, perihal, atau nama pelanggan…"
            value={search} onChange={e => setSearch(e.target.value)} autoFocus />

          {loading ? (
            <div className="text-center py-8 text-slate-400 text-sm">Memuat daftar dokumen…</div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-8 text-slate-400 text-sm">Tidak ada dokumen ditemukan.</div>
          ) : (
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4"
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg max-h-[80vh] flex flex-col shadow-2xl"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              Pilih RAB untuk mengisi otomatis perihal, item, instansi, dan data penerima
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
          <input className={inp} placeholder="🔍 Cari nomor atau perihal RAB…"
            value={search} onChange={e => setSearch(e.target.value)} autoFocus />

          {loading ? (
            <div className="text-center py-10 text-slate-400 text-sm">Memuat daftar RAB…</div>
          ) : error ? (
            ...
            <div className="text-center py-10 text-slate-400 text-sm">
              {list.length === 0 ? "Belum ada RAB yang tersimpan." : "Tidak ada RAB yang sesuai pencarian."}
            </div>
          ) : (
            <div className="space-y-2">
              {filtered.map(r => {
                ...
                return (
                  <button key={r.id} onClick={() => onSelect(r)}
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm font-semibold text-slate-800 group-hover:text-amber-800 transition-colors">
                        {r.nomor || "—"}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-1.5">
                      ...
                      <p className="text-xs font-semibold text-amber-700 ml-2 shrink-0">
                        {formatRupiah(total)}
                      </p>
                    </div>
                    {r.items?.length > 0 && (
                      <p className="text-[11px] text-slate-400 mt-0.5">{r.items.length} item</p>
                    )}
                  </button>
                );
              })}
