...
export default function RabPreview() {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      const available = wrapper.offsetWidth - padding;
...
`,
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const filename = `RAB-${data?.nomor || data?.perihal || data?.id || "dokumen"}.pdf`;
    ...
    <div className="bg-slate-200 min-h-screen pb-12">
      ...
      <div className="sticky top-0 z-10 bg-white border-b shadow-sm print:hidden">
        <div className="max-w-5xl mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between gap-2 sm:gap-3">
          ...
          {/* Mobile title — single line truncated */}
      ...
      <div ref={scrollWrapperRef} className="py-4 sm:py-6 px-2 sm:px-0">
        {/* Outer wrapper — height = scaled inner height supaya tidak ada whitespace */}
        <div
          style={{
        ...
        >
          {/* Scale wrapper — applies transform */}
          <div
            style={{
          ...
          >
            {/* Inner ref untuk measure tinggi asli */}
            <div ref={innerRef}>
              {/* printRef untuk export — ukuran asli */}
              <div ref={printRef}>
                <TemplateRabAdytia data={data} instansi={instansi} pageMargin={pdfMargin} fontScale={fontScale} />
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Floating zoom indicator di mobile */}
      {scale < 1 && (
...
function slug(s = "") {
  return String(s).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "").slice(0, 50) || "doc";
}
function dateTag() {
  const d = new Date();
