...
import {
  ...
  ChevronRight, RefreshCw, Receipt, Loader2, Download,
} from "lucide-react";
...
export default function MobileInvoice() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const type     = docData?.type === "invoice" ? "Invoice" : "RAB";
      const rawName  = `${type}-${docData?.nomor || docData?.perihal || "dokumen"}`;
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [inv, rab] = await Promise.all([
  ...
  const activeList = tab === "invoice" ? invoiceList : rabList;
  const visible    = filter === "all" ? activeList : activeList.filter(i => i.status === filter);
  ...
  const editPath = (item) =>
    item.type === "invoice"
      ? `/app-mobile/invoice/${item.id}`
      : `/app-mobile/rab/${item.id}`;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-100 px-4 py-3 flex items-center gap-3 pt-safe sticky top-0 z-10">
        ...
        <div className="flex-1">
          <h1 className="font-bold text-slate-800">Invoice & Cetak</h1>
          <p className="text-xs text-slate-400">
            {tab === "invoice" ? invoiceList.length : rabList.length} dokumen
          </p>
        </div>
        <button
          onClick={() => navigate("/app-mobile/invoice/baru")}
          className="flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white text-xs font-semibold rounded-xl active:scale-95 transition-transform">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex bg-slate-100 rounded-2xl p-1 gap-1">
          {[
            { key: "invoice", label: "Invoice", icon: <Receipt size={14} /> },
            { key: "rab",     label: "Dari RAB", icon: <FileText size={14} /> },
          ].map(({ key, label, icon }) => (
            <button key={key} onClick={() => { setTab(key); setFilter("all"); }}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition-all
                ${tab === key
                  ? key === "invoice" ? "bg-blue-600 text-white shadow-sm" : "bg-amber-500 text-white shadow-sm"
                  : "text-slate-500"}`}>
              {icon}{label}
            </button>
          ))}
          ...
          {["all", "draft", "final"].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              className={`shrink-0 px-3 py-1.5 rounded-xl text-xs font-medium transition-colors
                ${filter === f
                  ? tab === "invoice" ? "bg-blue-600 text-white" : "bg-amber-500 text-white"
                  : "bg-white border border-slate-200 text-slate-600"}`}>
              {f === "all"
                ? `Semua (${activeList.length})`
                : `${STATUS_LABEL[f] || f} (${activeList.filter(i => i.status === f).length})`}
            </button>
          ))}
          <button onClick={load}
        ...
        {!loading && visible.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-slate-400 text-sm font-medium">
              {tab === "invoice" ? "Belum ada invoice" : "Belum ada RAB"}
            </p>
            {tab === "invoice" && (
              <button onClick={() => navigate("/app-mobile/invoice/baru")}
            ...
            <div key={item.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-400 truncate mt-0.5">
                      {item.tagihan?.nama || item.tagihan?.perusahaan ||
                       item.kepada?.perusahaan || item.kepada?.yth || "—"}
                    </p>
                    <div className="flex items-center gap-3 mt-2">
                      <span className={`text-sm font-bold ${tab === "invoice" ? "text-blue-600" : "text-amber-600"}`}>
              ...
              <div className="border-t border-slate-100 flex">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-semibold transition-colors ${btnColor}`}>
                  <Eye size={15} />
                  Preview & Cetak
                </button>
      ...
      {previewDoc && (
        ...
        >
          {previewDoc.type === "invoice"
            ? <TemplateInvoiceAdytia data={previewDoc} instansi={previewInstansi} pageMargin={pdfMargin} fontScale={fontScale} />
          ...
          <div className="mobile-preview-chrome flex items-center gap-3 px-4 py-3 border-b border-slate-200 bg-white shrink-0 pt-safe"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400 truncate">
                {previewDoc.type === "invoice" ? "Invoice" : "RAB"} · {previewDoc.nomor || "—"}
              </p>
            </div>
            <button onClick={handleDownload}
              ...
              style={{ background: previewDoc.type === "invoice" ? "#2563EB" : "#003087" }}>
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[60] bg-white rounded-t-3xl px-4 pt-3 pb-safe-sheet">
            ...
            <p className="text-sm font-semibold text-slate-800 mb-1">
              Hapus {delTarget.type === "invoice" ? "invoice" : "RAB"} ini?
            </p>
